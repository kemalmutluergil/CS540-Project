...
# =====================================================================
# 🛠️ SUSTAINABILITY ADDITION: Live Digital Sensor Pipeline
# =====================================================================
class LiveDigitalSensorPipeline:
    """
    Simulates an asynchronous external legal sensor registry feed.
    Evaluates incoming raw payload events, flags obsolete records,
    and commits non-destructive state mutations to the vector store.
    """
    @staticmethod
    def process_incoming_statute(text, doc_name, uploader_name, repealed_statute_id=""):
        sustainability_msg = ""
        
        # Detection Phase: Sensor checks if an older law is being repealed or superseded
        if repealed_statute_id.strip():
            old_docs = collection.get(ids=[repealed_statute_id.strip()])
            if old_docs and old_docs['ids']:
                old_metadata = old_docs['metadatas'][0]
                
                # Mutation Phase: State-mutation from True to False (Non-destructive)
                old_metadata['is_obsolete'] = "True"
                old_metadata['superseded_by'] = doc_name
                old_metadata['deactivation_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        collection.add(
            ...
            metadatas=[{
                ...
                "is_obsolete": "False",  # Default logically to active operational state
                "superseded_by": "None"
            }],
            ids=[doc_name]
        )
        return f"Successfully indexed '{doc_name}' into secure vault.{sustainability_msg}"
...
def add_document(text, doc_name, uploader_name, supersedes_name=""):
    ...
    # System strictly routes through the formal Live Digital Sensor Pipeline logic
    return LiveDigitalSensorPipeline.process_incoming_statute(text, doc_name, uploader_name, supersedes_name)
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Ask the Co-Pilot"):
        ...
        # Upper UI Element for Pre-processing alerts (Preserved exactly as requested)
        ...
        with gr.Row():
            # Lower UI Elements for alerts (Preserved exactly as requested)
            proxy_alert_output_lower = gr.Textbox(label="Pre-Processing Bias Check", interactive=False)
            sustain_alert_output = gr.Textbox(label="Sustainability Monitor", interactive=False)
          
        # Map inputs and outputs precisely to maintain interface alignment without compression
        ask_btn.click(
            ask_copilot, 
            inputs=[query_input], 
            outputs=[draft_output, drivers_output, confidence_output, fairness_output, proxy_alert_output_lower, source_output, hidden_query, hidden_source, sustain_alert_output]
        ).then( 
    ...
    with gr.Tab("Upload Documents"):
        gr.Markdown("### ♻️ Automated Live Registry Input (Digital Sensor Active)")
        ...
        # Add the supersedes input field
        ...
        # Directly runs through the Live Digital Sensor implementation wrapper
